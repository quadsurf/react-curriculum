...
Imagine that components A, B, and C are arranged into the following hierarchy. If the state of component C changes from `Charizard` to `Bulbasaur`, React can update only the part of the DOM that the component represents. In this case, only the contents of the `<div id="C">` element is updated. In a moment, you'll see a concrete example of how this works in code.
